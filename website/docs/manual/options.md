...
Another possibility is to draw one of the following three types of samples from the population (see the examples and suggestions in [Haunschild et al., 2020](https://doi.org/10.1177/0165551519837175)):
...
* **Cluster Sampling**: From the years which are in-between the minimum and maximum publication years (see above) one citing year is randomly selected. Then, all CRs in the papers published in this year are selected as sample and imported. The results of [Bornmann and Mutz (2015)](https://doi.org/10.1002/asi.23329) reveal that the restriction on all CRs from a recent citing year leads to very similar results as the consideration of all CRs from several citing years in references analysis. Note that the selection of one citing year in CRExplorer implicates that citation distributions over many citing years cannot be analyzed.

Of course, the three sampling options can be combined with the selection of citing and/or cited years. Furthermore, many samples instead of one sample can be drawn from the population to optimize the RPYS results ([Haunschild et al., 2020](https://doi.org/10.1177/0165551519837175)).
...
The analysis of a sample instead of the population is only an opportunity to handle the restrictions, which are given by the computer of the user. It is always the best way in the cited references analysis to analyze the population (by running CRExplorer with the database option). If the user, however, decides to work with a sample instead of the population, inference statistics can be applied to draw conclusions from the sample to the population. For using inference statistics, the user can download the sample data as csv-file from CRExplorer. Using statistics programs, such as Stata (see [www.stata.com]()) or R (see [www.r-project.org]()), the user can apply inference statics to the exported (random) sample ([Cumming, 2012](https://doi.org/10.4324/9780203807002)). In the SSC Archive (Stata), the user finds the Stata command [plotrpys](https://EconPapers.repec.org/RePEc:boc:bocode:s458378) which produces monochrome and colored graphs including 95% confidence intervals. These graphs can be used in connection with sample data.
...
If there are any problems with changing of chart types, try **View** – **Reset Chart** or restart CRExplorer.
