...
/// caption
Figure 1. Screenshot of CRExplorer
///
...
* **N_PYEARS (Number of Citing Years)**: Several CRs are cited in more than one publication with different PYs. N_PYEARS shows the number of different PYs in which the CR has been cited.
* **PERC_PYEARS (Percentage of Citing Years)**: The publication set which is analyzed by CRExplorer includes as a rule publications from different PYs. PERC_PYEARS shows the percentage of years (citing years), in which the CR was cited at least once. Thus, N_PYEARS is divided by the maximal number of citing years (i.e., all PYs with at least one mention of a CR from the specific RPY).
* **CP-IN (Cumulative Percentage including Ties)**: Represents the percentage of CRs within an RPY that have a lower or equal citation frequency (N_CR) as the focal CR in the sorted distribution. Papers sharing the same N_CR value are included in this cumulative total ([Bornmann & Williams, 2020](https://doi.org/10.1007/s11192-020-03512-7)). The indicator can be used to identify CRs under a significant peak that — relative to the other CRs under that peak — exhibit particularly high citation impact. For instance, a user could interpret all CRs under significant peaks in a spectrogram that have a CP-IN value of 80 or higher.
* **CP-EX (Cumulative Percentage excluding Ties)**: Represents the percentage of references within an RPY that have a lower citation frequency (N_CR) as the focal CR in the sorted distribution. Papers sharing the same N_CR value are excluded from this cumulative total ([Bornmann & Williams, 2020](https://doi.org/10.1007/s11192-020-03512-7)). The indicator can be used to identify CRs under a significant peak that — relative to the other CRs under that peak — exhibit particularly high citation impact. For instance, a user could interpret all CRs under significant peaks in a spectrogram that have a CP-EX value of 80 or higher.
* * **CPIMP-IN (Cumulative Impact Percentage including Ties)**: Represents the percentage of impact (N_CR) within an RPY that have a higher or equal citation frequency (N_CR) as the focal CR in the sorted distribution. Papers sharing the same N_CR value are included in this cumulative total ([Bornmann & Williams, 2020](https://doi.org/10.1007/s11192-020-03512-7)). The indicator can be used to identify CRs under a significant peak that — relative to the other CRs under that peak — exhibit particularly high citation impact. For instance, a user could interpret all CRs under significant peaks in a spectrogram that have a CPIMP-IN value of 80 or higher.
* **CPIMP-EX (Cumulative Impact Percentage excluding Ties)**: Represents the percentage of impact (N_CR) within an RPY that have a higher citation frequency (N_CR) in the sorted distribution. Papers sharing the same N_CR value are excluded from this cumulative total ([Bornmann & Williams, 2020](https://doi.org/10.1007/s11192-020-03512-7)). The indicator can be used to identify CRs under a significant peak that — relative to the other CRs under that peak — exhibit particularly high citation impact. For instance, a user could interpret all CRs under significant peaks in a spectrogram that have a CPIMP-EX value of 80 or higher.
* **N_TOP50, N_TOP25, N_TOP10 (top 50%, 25%, 10% Cited Reference)**: These indicators can be used to identify those CRs which have been cited more frequently in the citing years than other CRs in the dataset ([Thor, et al., 2018](https://doi.org/10.1007/s11192-018-2733-7)). In order to identify these CRs, thresholds are computed which identify the top 50%, top 25%, and top 10% in one citing year. In the first step of the computation, the citations in one citing year are sorted in ascending order. In the second step, the thresholds for the top 50%, 25%, and 10% are determined in a given year. In the third step, those CRs are identified which are above the three thresholds. In the fourth step, the numbers of citing years are counted in which the CRs are above the thresholds. These numbers yield N_TOP50, N_TOP25, and N_TOP10.
* **Sequence and Type**: In order to identify statistically the citation dynamic of CRs over time, we apply Configural Frequency Analysis (CFA, [Stemmler, 2014](https://link.springer.com/book/10.1007/978-3-030-49421-6); [von Eye, 2002](https://doi.org/10.4324/9781410606570)). CFA is a categorical statistical procedure to reveal configurations in multivariate cross-classifications (i.e., contingency tables). CRExplorer cross-classifies the CRs for a certain RPY and the PY for the citing publications. In the case of systematic citation dynamics (e.g., if a publication belongs to the “sleeping beauty” type) citations in the cells deviate strongly from the expected values. Expected values are cell frequencies, which would occur if there is no relationship between or independency of the row (CRs) and the column variable (publication year). The approach of calculating expected values and deviations from these values is explained in detail in [Thor et al., 2018](https://doi.org/10.1007/s11192-018-2733-7)).
