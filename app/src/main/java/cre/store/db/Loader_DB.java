...
public class Loader_DB implements Loader {
	...
	public Loader_DB(Connection dbCon) throws SQLException {
		...
		pst_OnNewCR = dbCon.prepareStatement(Queries.getQuery("Loader_DB", "on_new_cr").get(0));
		pst_OnNewCR_Counter = 0;
		
		pst_OnNewPub = dbCon.prepareStatement(Queries.getQuery("Loader_DB", "on_new_pub").get(0));
		pst_OnNewPub_Counter = 0;
		
		pst_OnNewPub_CR = dbCon.prepareStatement(Queries.getQuery("Loader_DB", "on_new_pub_cr").get(0));
		pst_OnNewPub_CR_Counter = 0;
		
		pst_OnNew_MatchPair_Manu = dbCon.prepareStatement(Queries.getQuery("Loader_DB", "on_new_matchpair_manu").get(0));
		pst_OnNew_MatchPair_Manu_Counter = 0;
		
		pst_OnNew_MatchPair_Auto = dbCon.prepareStatement(Queries.getQuery("Loader_DB", "on_new_matchpair_auto").get(0));
	...
	public void onNewPub(PubType_MM pub, List<Integer> crIds) {
		...
		try {
			...
			pst_OnNewPub.setString 	( 3, pub.getAU().count() == 0 ? null : pub.getAU().collect(PubType_DB.joinByNewLine)); 
			pst_OnNewPub.setString 	( 4, pub.getAF().count() == 0 ? null : pub.getAF().collect(PubType_DB.joinByNewLine)); 
			pst_OnNewPub.setString 	( 5, pub.getC1().count() == 0 ? null : pub.getC1().map(it -> String.join(PubType_DB.arraySeparator2, it)).collect(PubType_DB.joinByNewLine)); 
			pst_OnNewPub.setString 	( 6, pub.getEM().count() == 0 ? null : pub.getEM().collect(PubType_DB.joinByNewLine)); 
			pst_OnNewPub.setString 	( 7, pub.getAA().count() == 0 ? null : pub.getAA().collect(PubType_DB.joinByNewLine)); 
